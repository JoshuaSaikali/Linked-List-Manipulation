...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		if (p == NULL)
			printf("no patient with priority %d\n", i);
		else
			printPatient(&patient);
	}

	// checking number of patients by criteria
	printf("\nprinting the patients by criteria \n");
	ListNode *j = searchNextPatient(patientsList, findPatient, &patient);
	if (j == NULL)
		printf("no patient by criteria\n");
	else
		printPatient(&patient);
	...
	printf("printing list after removing priority <= 3 \n");
	releasePatients(&patientsList, 3);

	// testing the list revers function
	printf("Printing List after reverse\n");
	...
	// for (i = 0; i < NUMBER_OF_TEST_PATIENTS; i++)
	// {
	// 	deleteFromList(&patientsList, &patient);
